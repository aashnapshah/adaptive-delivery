"""Delivery interface and clinician response (stage 05).

A scored recommendation only creates value if it is delivered well and the clinician acts on it. This
stage simulates the human in the loop across the five presentation actions the policy uses - interrupt,
passive, delay, bundle, suppress - and encodes alert fatigue: acceptance falls as recent alert burden
grows, and each action adds to that burden in proportion to how intrusive it is (AHRQ 2019; Park 2022;
Goh 2024).
...
class ClinicianSimulator:
    ...
    alert-fatigue effect - falls as the running `burden` grows. Salience scales acceptance by how
    attention-grabbing the action is; intrusive actions also add more burden.
    """
...
def main() -> None:
    ...
    print("\nThe policy (stage 06) chooses among the five actions to keep useful alerts accepted "
